...
class User extends Allow
{
    ...
    public function getUser()
    {
        ...
        for($i = 0 ; $i<count($data) ; $i++){
            $data[$i]['found_time'] = date('Y-m-d',$data[$i]['found_time']);
            $data[$i]['visit_time'] = date('Y-m-d H:i',$data[$i]['visit_time']);
        ...
        $arr['pro']         = $pro;                     //当前页
        $arr['row']         = $data;                    //遍历数据
        $arr['title']       = '用户管理';              //主目录名称
        $arr['title_txt']   = '用户列表';              //子目录名称
        ...
        return $this->fetch('User/user',$arr);
    }
    ...
    public function getAdminadd()
    {
        ...
        if(empty($id)){
            ...
            $title_txt  = '用户修改';
            $data        = Db::table($table)->where('id',$id)->find();        //找出用户
