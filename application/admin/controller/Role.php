...
class Role extends Allow
{
    ...
    public function getRole()
    {
        ...
        //图片
        // $img  = Db::table($this->dbimg)->where('img_surface_name',$table)->select();        //获取所有角色图片
        ...
        $arr['pro']         = $pro;                     //当前页
        $arr['row']         = $data;                    //遍历数据
        $arr['title']       = '角色管理';              //主目录名称
        $arr['title_txt']   = '角色列表';              //子目录名称
        ...
        return $this->fetch('Role/role',$arr);
    }


    //添加功能
    public function postRoleDoAdd()
    {
        $id                     = input('id');                 //id
        $data['title']     = input('title');         //用户账号

        $table = $this->dbt.'admin_role';
        //判断是否修改或添加
        if(!empty($id)){
